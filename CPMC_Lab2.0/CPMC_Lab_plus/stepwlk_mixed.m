function [phi, w, O, E, W, C1C2, S1S2, C1C2C, E_K_mixed, E_V_mixed] = stepwlk_mixed(phi, N_wlk, N_sites, w, O, E, W, C1C2, S1S2, C1C2C, E_K_mixed, E_V_mixed, H_k, Proj_k_half, flag_mea, Phi_T, N_up, N_par, U, fac_norm, aux_fld)
...
s1s2=zeros(N_wlk,1); 
c1c2c=zeros(N_wlk,1);
e_k_mixed=zeros(N_wlk,1);
e_v_mixed=zeros(N_wlk,1);
    ...
    if w(i_wlk)>0
        ...
        if w(i_wlk)>0
            ...
            if w(i_wlk)>0
                ...
                if flag_mea==1
                    [e(i_wlk), c1c2(i_wlk), s1s2(i_wlk), c1c2c(i_wlk), e_k_mixed(i_wlk), e_v_mixed(i_wlk)]=measure_mixed(H_k, Phi(:,:), Phi_T,  invO_matrix_up, invO_matrix_dn, N_up, N_par, U);
                end
            end
        end
    end
...
if flag_mea==1
    for i_wlk=1:N_wlk
        if w(i_wlk)>0
            C1C2=C1C2+c1c2(i_wlk)*w(i_wlk); 
            S1S2=S1S2+s1s2(i_wlk)*w(i_wlk); 
            C1C2C=C1C2C+c1c2c(i_wlk)*w(i_wlk); 
            E_K_mixed=E_K_mixed+e_k_mixed(i_wlk)*w(i_wlk); 
            E_V_mixed=E_V_mixed+e_v_mixed(i_wlk)*w(i_wlk); 
            E=E+e(i_wlk)*w(i_wlk);  
            W=W+w(i_wlk);
        end
    end
