...
U1=[U0-N_deltaU*deltaU:deltaU:U0+N_deltaU*deltaU];
...
N_run=length(U1); %replace argument by the parameter that needs to be looped over
E_ave=zeros(N_run,1);
E_err=zeros(N_run,1);

%% invoke the main function
for i=1:N_run
    suffix=strcat('_U1',int2str(U1(i))); % Set the suffix to distinguish between different runs in the same batch    
    % call main function AFQMC_Hub
    [E_ave(i),E_err(i),savedFile]=CPMC_Lab_NiNi(Lx,Ly,Lz,N_up,N_dn,kx,ky,kz,U,U1(i),tx,ty,tz,deltau,N_wlk,N_blksteps,N_eqblk,N_blk,itv_modsvd,itv_pc,itv_Em,suffix);    
end
%% post-run:
...
for i=1:N_deltaU
    E_NiNi(i,i_N_loop)=(E_ave(2+2*N_deltaU-i)-E_ave(i))./(2*(1+N_deltaU-i)*deltaU);
end
...
% plot E_NiNi 
for i=1:N_deltaU
    E_NiNi_avg(i_U,i)=mean(E_NiNi(i,:));
    E_NiNi_err(i_U,i)=std(E_NiNi(i,:))/sqrt(N_loop);
end
E_NiNi_AVG(i_U)=mean(E_NiNi_avg(i_U,:));
E_NiNi_ERR(i_U)=mean(E_NiNi_err(i_U,:));
%% end U loop, icf 2017/9/20
end

figure;
errorbar(1*(1:N_U_loop),E_NiNi_AVG,E_NiNi_ERR); 
xlabel ('U0');
ylabel ('E_NiNi');
