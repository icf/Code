function [Phi, Phi_save, x, w, O]=pop_cntrl_BP(Phi, Phi_save, x, w, O, N_wlk, N_sites, N_par, itv_Em)
...
% "CPMC-Lab: A Matlab Package for Constrained Path Monte Carlo Calculations" Comput. Phys. Commun. (2014)
    ...
    new_Phi_save=zeros(N_sites, N_par, N_wlk); %in the end the number of walkers will still be N_wlk
    new_x=zeros(N_sites,itv_Em,N_wlk);; %in the end the number of walkers will still be N_wlk
    ...
    for i_wlk=1:N_wlk
        ...
        for j=(n_wlk+1):n
            new_Phi(:,:,j)=Phi(:,:,i_wlk);
            %Bp popcontrol, icf 2017/10/3
            new_Phi_save(:,:,j)=Phi_save(:,:,i_wlk);
            new_x(:,:,j)=x(:,:,i_wlk);
            %
            new_O(j)=O(i_wlk);
        end
