function [phi, w, O, E, E_bp, E_V_bp, E_K_bp, G_bp, W, x, phi_save] = stepwlk_AP_G(phi, phi_save, N_wlk, N_sites, w, O, E, E_bp, E_V_bp, E_K_bp, G_bp, W, H_k, Proj_k_half, flag_mea, Phi_T, N_up, N_par, U, fac_norm, aux_fld, x, steps, itv_Em, itv_modsvd)
    ...
    if w(i_wlk)>0
        ...
        if w(i_wlk)>0
            ...
            if w(i_wlk)>0
                ...
                if flag_mea==1
                    x(:,steps,i_wlk)=X;
                    [e_bp(i_wlk),e_V_bp(i_wlk),e_K_bp(i_wlk),g_bp(i_wlk,:,:)]=measure_BP_G(H_k, phi_save(:,:,i_wlk), Phi_T, N_sites, N_up, N_par, U, x(:,:,i_wlk), Proj_k_half, itv_Em, aux_fld, itv_modsvd);
