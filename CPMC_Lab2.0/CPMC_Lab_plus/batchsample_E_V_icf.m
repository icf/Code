...
N_deltaU=1; % N_deltaU has to be 1 or E_V_ERR won't give the right estimation.
deltaU=0.16;
N_loop=10;
...
U=[U0-N_deltaU*deltaU:deltaU:U0+N_deltaU*deltaU]; % The on-site repulsion strength in the Hubbard Hamiltonian
...
N_run=length(U); %replace argument by the parameter that needs to be looped over
E_ave=zeros(N_run,1);
E_err=zeros(N_run,1);

%% invoke the main function
for i=1:N_run
    suffix=strcat('_U',int2str(U(i))); % Set the suffix to distinguish between different runs in the same batch    
    % call main function AFQMC_Hub
    [E_ave(i),E_err(i),savedFile]=CPMC_Lab(Lx,Ly,Lz,N_up,N_dn,kx,ky,kz,U(i),tx,ty,tz,deltau,N_wlk,N_blksteps,N_eqblk,N_blk,itv_modsvd,itv_pc,itv_Em,suffix);    
end
%% post-run:
...
% xlabel ('U');
% ylabel ('E');
% calculate and save E_V, icf 2017/9/19
U_ave=mean(U); 
for i=1:N_deltaU
    E_V(i,i_N_loop)=U_ave*(E_ave(2+2*N_deltaU-i)-E_ave(i))./(2*(1+N_deltaU-i)*deltaU);
end
%% end the loop, icf 2017/9/19
end

%% sum up, icf 2017/9/19
for i=1:N_deltaU
    E_V_avg(i_U,i)=mean(E_V(i,:));
    E_V_err(i_U,i)=std(E_V(i,:))/sqrt(N_loop);
end
E_V_AVG(i_U)=mean(E_V_avg(i_U,:));
E_V_ERR(i_U)=mean(E_V_err(i_U,:));
%% end U loop, icf 2017/9/20
end

figure;
errorbar(1*(1:N_U_loop),E_V_AVG,E_V_ERR); 
xlabel ('U0');
ylabel ('E_V');
