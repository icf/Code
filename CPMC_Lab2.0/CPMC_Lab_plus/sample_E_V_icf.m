...
U=[4.0-N_deltaU*deltaU:deltaU:4.0+N_deltaU*deltaU]; % The on-site repulsion strength in the Hubbard Hamiltonian
...
for i=1:N_run
    suffix=strcat('_N_wlk',int2str(U(i))); % Set the suffix to distinguish between different runs in the same batch    
...
for i=1:N_deltaU
    E_V_avg(i)=mean(E_V(i,:));
    E_V_err(i)=std(E_V(i,:))/sqrt(N_loop);
end

figure;
errorbar(2*(1+N_deltaU-(1:N_deltaU))*deltaU,E_V_avg(:),E_V_err(:));
xlabel ('2*(1+N_deltaU-(1:N_deltaU)):the distance');
ylabel ('E_V');
