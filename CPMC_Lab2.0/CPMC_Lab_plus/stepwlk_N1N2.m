function [phi, w, O, E, W] = stepwlk_N1N2(phi, N_wlk, N_sites, w, O, E, W, H_k, Proj_k_half, flag_mea, Phi_T, N_up, N_par, U, U12, fac_norm, aux_fld, aux_fld12)
    ...
    if w(i_wlk)>0
        ...
        if w(i_wlk)>0
            ...
            for j_site=1:N_sites
                if w(i_wlk)>0
                   if j_site==1 
                       [Phi(j_site,:), Phi(j_site+1,:), O(i_wlk), w(i_wlk), invO_matrix_up, invO_matrix_dn]=V12(Phi(j_site,:), Phi(j_site+1,:), Phi_T(j_site,:), Phi_T(j_site+1,:), N_up, N_par, O(i_wlk), w(i_wlk), invO_matrix_up, invO_matrix_dn, aux_fld12);
        ...
        if w(i_wlk)>0
            ...
            if w(i_wlk)>0
                ...
                if flag_mea==1
                    e(i_wlk)=measure_N1N2(H_k, Phi(:,:), Phi_T,  invO_matrix_up, invO_matrix_dn, N_up, N_par, U, U12);
                end
            end
        end
    end
