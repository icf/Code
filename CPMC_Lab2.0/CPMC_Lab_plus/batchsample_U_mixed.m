...
U=[0.1:0.1:8.1]; % The on-site repulsion strength in the Hubbard Hamiltonian
...
for i=1:N_run
    ...
    [E_ave(i),E_err(i),C1C2_ave(i),C1C2_err(i),S1S2_ave(i),S1S2_err(i),C1C2C_ave(i),C1C2C_err(i),E_K_mixed_ave(i),E_K_mixed_err(i),E_V_mixed_ave(i),E_V_mixed_err(i),savedFile]=CPMC_Lab_mixed(Lx,Ly,Lz,N_up,N_dn,kx,ky,kz,U(i),tx,ty,tz,deltau,N_wlk,N_blksteps,N_eqblk,N_blk,itv_modsvd,itv_pc,itv_Em,suffix);    
end
%% post-run:
...
figure;
errorbar(U,C1C2_ave,C1C2_err);
xlabel ('U');
ylabel ('C1C2');

figure;
errorbar(U,S1S2_ave,S1S2_err);
xlabel ('U');
ylabel ('S1S2');

figure;
errorbar(U,C1C2C_ave,C1C2C_err);
xlabel ('U');
ylabel ('C1C2C');

figure;
errorbar(U,E_K_mixed_ave,E_K_mixed_err);
xlabel ('U');
ylabel ('E_K_mixed');

figure;
errorbar(U,E_V_mixed_ave,E_V_mixed_err);
xlabel ('U');
ylabel ('E_V_mixed');
