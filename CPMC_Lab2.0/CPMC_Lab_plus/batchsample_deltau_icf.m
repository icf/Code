...
%% run parameters:
deltau=[0.01:0.005:0.3]; % The imaginary time step
N_wlk=100; % The number of random walkers
N_blksteps=round(600*0.01./deltau); % The number of random walk steps in each block
N_eqblk=3; % The number of blocks used to equilibrate the random walk before energy measurement takes place
...
itv_Em=round(100*0.01./deltau); % The interval between two adjacent energy measurements

%% Initialize the batch run
N_run=length(deltau); %replace argument by the parameter that needs to be looped over
E_ave=zeros(N_run,1);
E_err=zeros(N_run,1);

%% invoke the main function
for i=1:N_run
    suffix=strcat('_deltau',int2str(deltau(i))); % Set the suffix to distinguish between different runs in the same batch    
    % call main function AFQMC_Hub
    [E_ave(i),E_err(i),savedFile]=CPMC_Lab(Lx,Ly,Lz,N_up,N_dn,kx,ky,kz,U,tx,ty,tz,deltau(i),N_wlk,N_blksteps(i),N_eqblk,N_blk,itv_modsvd,itv_pc,itv_Em(i),suffix);    
end
%% post-run:
% plot energy vs different run parameters
figure;
errorbar(deltau,E_ave,E_err);
xlabel ('deltau');
ylabel ('E');
