function [e, c1c2, s1s2, c1c2c, e_k_mixed, e_v_mixed] = measure_mixed(H_k, phi, Phi_T, invO_matrix_up, invO_matrix_dn, N_up, N_par, U)
...
% "CPMC-Lab: A Matlab Package for Constrained Path Monte Carlo Calculations" Comput. Phys. Commun. (2014)
    ...
    e_v_mixed=potentialEnergy;
    
    %% calculate one body density matrix, spin-spin, charge-charge, icf 2017/9/24
    c1c2=G_up(2,1);
    s1s2=G_up(1,1).'*G_up(2,2)+G_dn(1,1).'*G_dn(2,2)-G_up(1,1).'*G_dn(2,2)-G_dn(1,1).'*G_up(2,2);
    c1c2c=G_up(1,1).'*G_up(2,2)+G_dn(1,1).'*G_dn(2,2)+G_up(1,1).'*G_dn(2,2)+G_dn(1,1).'*G_up(2,2);
    
    %% calculate the kinetic energy:
    kineticEnergy=sum(sum(H_k.'.*(G_up+G_dn))); % note the element-wise multiplication
    e_k_mixed=kineticEnergy;
