function [E_ave,E_err,E_BP_ave,E_BP_err,E_V_BP_ave,E_V_BP_err,E_K_BP_ave,E_K_BP_err,G_BP_ave_T,G_BP_err_T,savedFileName]=CPMC_Lab_G_X_pin(Lx,Ly,Lz,N_up,N_dn,kx,ky,kz,U,tx,ty,tz,deltau,N_wlk,N_blksteps,N_eqblk,N_blk,itv_modsvd,itv_pc,itv_Em,suffix,phi_2)
...
%% Initialization
tic; % start the  timer
initialization_X_pin; % initialize internal constants, form the trial wave function and assemble the initial population of walkers
format long;
...
G_BP_blk=zeros(N_blk,N_sites,2*N_sites);
...
for i_blk=1:N_eqblk    
    for j_step=1:N_blksteps
        [Phi, w, O, E, W] = stepwlk_pin(Phi, N_wlk, N_sites, w, O, E, W, H_k, Proj_k_half, Proj_pin_up_half, Proj_pin_dn_half, flag_mea, Phi_T, N_up, N_par, U, fac_norm, aux_fld);
        %detect the Mixed Energy, icf 2017/9/17
...
for i_blk=1:N_blk
    for j_step=1:N_blksteps
        ...
        [Phi, w, O, E_blk(i_blk), E_BP_blk(i_blk), E_V_BP_blk(i_blk), E_K_BP_blk(i_blk), G_BP_blk(i_blk,:,:), W_blk(i_blk), x_save] = stepwlk_AP_G_pin(Phi, Phi_save, N_wlk, N_sites, w, O, E_blk(i_blk), E_BP_blk(i_blk), E_V_BP_blk(i_blk), E_K_BP_blk(i_blk), G_BP_blk(i_blk,:,:), W_blk(i_blk), H_k, Proj_k_half, Proj_pin_up_half, Proj_pin_dn_half, flag_mea, Phi_T, N_up, N_par, U, fac_norm, aux_fld, x_save, steps, itv_Em, itv_modsvd);
        %
    ...
    G_BP_blk(i_blk,:,:)=G_BP_blk(i_blk,:,:)./W_blk(i_blk);
...
G_BP=real(G_BP_blk);
G_BP_ave=mean(G_BP);
G_BP_err=std(G_BP)./sqrt(N_blk);
G_BP_ave_T=G_BP_ave(1,:,:);
G_BP_err_T=G_BP_err(1,:,:);
