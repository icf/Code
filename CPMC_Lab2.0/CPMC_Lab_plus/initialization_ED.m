...
%% ED N_up=N_dn=1, icf 2017/10/10
H_ED=zeros(16,16);
H_V_ED=zeros(16,16);
H_K_ED=zeros(16,16);
Phi_T_ED=zeros(16,1);
e_H_ED=zeros(16,16);
H_ED_value_ED=zeros(16,16);
H_ED_state_ED=zeros(16,16);
E_ED_mixed=0;
E_V_ED_mixed=0;
E_K_ED_mixed=0;
I=zeros(4,4);
...
Phi_T_ED=kron(psi_nonint(:,1),psi_nonint(:,1))
H_K_ED=kron(H_k,I)+kron(I,H_k);
for i=1:16
    if (i+4)/5==fix((i+4)/5)  
    H_V_ED(i,i)=U;
    end
...
H_ED=H_V_ED+H_K_ED;
e_H_ED=expm(-1*itv_Em.*deltau.*H_ED);
[H_ED_state_ED,H_ED_value_ED] = eig(H_ED);
Phi_0_ED=H_ED_state_ED(:,1);
% 
E_K_ED_mixed=(Phi_T_ED'*e_H_ED*H_K_ED*Phi_0_ED)/(Phi_T_ED'*e_H_ED*Phi_0_ED)
E_V_ED_mixed=(Phi_T_ED'*e_H_ED*H_V_ED*Phi_0_ED)/(Phi_T_ED'*e_H_ED*Phi_0_ED)

E_K_ED=(Phi_0_ED'*e_H_ED*H_K_ED*Phi_0_ED)/(Phi_0_ED'*e_H_ED*Phi_0_ED)
E_V_ED=(Phi_0_ED'*e_H_ED*H_V_ED*Phi_0_ED)/(Phi_0_ED'*e_H_ED*Phi_0_ED)
E_ED=(Phi_T_ED'*e_H_ED*H_ED*Phi_0_ED)/(Phi_T_ED'*e_H_ED*Phi_0_ED);
%% plot Phi_T_ED, icf 2017/20/22
Phi_n_ED=conj(Phi_T_ED).*Phi_T_ED;
Phi_T_ED=Phi_T_ED
x=1:1:Lx;
y=1:1:Ly;
z=zeros(1,Lx*Ly);
