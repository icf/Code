function [E_ave,E_err,savedFileName]=CPMC_Lab_N1N2(Lx,Ly,Lz,N_up,N_dn,kx,ky,kz,U,U12,tx,ty,tz,deltau,N_wlk,N_blksteps,N_eqblk,N_blk,itv_modsvd,itv_pc,itv_Em,suffix)
...
%% Initialization
tic; % start the  timer
initialization_N1N2; % initialize internal constants, form the trial wave function and assemble the initial population of walkers
format long;
...
for i_blk=1:N_eqblk    
    for j_step=1:N_blksteps
        [Phi, w, O, E, W] = stepwlk_N1N2(Phi, N_wlk, N_sites, w, O, E, W, H_k, Proj_k_half, flag_mea, Phi_T, N_up, N_par, U, U12, fac_norm, aux_fld, aux_fld12);
...
for i_blk=1:N_blk
    for j_step=1:N_blksteps
        ...
        [Phi, w, O, E_blk(i_blk), W_blk(i_blk)] = stepwlk_N1N2(Phi, N_wlk, N_sites, w, O, E_blk(i_blk), W_blk(i_blk), H_k, Proj_k_half, flag_mea, Phi_T, N_up, N_par, U, U12, fac_norm, aux_fld, aux_fld12);
