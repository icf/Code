...
tx=[1.0-N_deltaU*deltaU:deltaU:1.0+N_deltaU*deltaU]; % The hopping amplitude between nearest-neighbor sites in the x direction
...
E_K=E_Ky+E_Kx;
for i=1:N_deltaU
    E_K_avg(i)=mean(E_K(i,:));
    E_K_err(i)=std(E_K(i,:))/sqrt(N_loop);
end

errorbar(2*(1+N_deltaU-(1:N_deltaU))*deltaU,E_K_avg,E_K_err);
