function [E_ave,E_err,E_BP_ave,E_BP_err,E_V_BP_ave,E_V_BP_err,E_K_BP_ave,E_K_BP_err,G_BP_ave_T,G_BP_err_T,savedFileName]=CPMC_Lab_G_HF(Lx,Ly,Lz,N_up,N_dn,kx,ky,kz,U,tx,ty,tz,deltau,N_wlk,N_blksteps,N_eqblk,N_blk,itv_modsvd,itv_pc,itv_Em,suffix)
...
%% Initialization
tic; % start the  timer
initialization_HF; % initialize internal constants, form the trial wave function and assemble the initial population of walkers
format long;
...
for i_blk=1:N_blk
    for j_step=1:N_blksteps
        ...
        [Phi, w, O, E_blk(i_blk), E_BP_blk(i_blk), E_V_BP_blk(i_blk), E_K_BP_blk(i_blk), G_BP_blk(i_blk,:,:), W_blk(i_blk), x_save] = stepwlk_AP_G(Phi, Phi_save, N_wlk, N_sites, w, O, E_blk(i_blk), E_BP_blk(i_blk), E_V_BP_blk(i_blk), E_K_BP_blk(i_blk), G_BP_blk(i_blk,:,:), W_blk(i_blk), H_k, Proj_k_half, flag_mea, Phi_T, N_up, N_par, U, fac_norm, aux_fld, x_save, steps, itv_Em, itv_modsvd);
        %
