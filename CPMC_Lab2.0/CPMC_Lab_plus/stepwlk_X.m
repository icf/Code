function [phi, w, O, E, PROJ, W] = stepwlk_X(phi, N_wlk, N_sites, w, O, E, PROJ, W, H_k, Proj_k_half, flag_mea, Phi_T, Phi_T2, N_up, N_par, U, fac_norm, aux_fld)
...
for i_wlk=1:N_wlk
    ...
    if w(i_wlk)>0
        ...
        [Phi, w(i_wlk), O(i_wlk), invO_matrix_up, invO_matrix_dn, invO_matrix_up2, invO_matrix_dn2]=halfK_X(Phi, w(i_wlk), O(i_wlk), Proj_k_half, Phi_T, Phi_T2, N_up, N_par);
        if w(i_wlk)>0
            % propagate each lattice site of a walker by the potential term:
            for j_site=1:N_sites
                if w(i_wlk)>0
                    [Phi(j_site,:), O(i_wlk), w(i_wlk), invO_matrix_up, invO_matrix_dn, invO_matrix_up2, invO_matrix_dn2]=V_X(Phi(j_site,:), Phi_T(j_site,:), Phi_T2(j_site,:), N_up, N_par, O(i_wlk), w(i_wlk), invO_matrix_up, invO_matrix_dn, invO_matrix_up2, invO_matrix_dn2, aux_fld);
                end
            end
        end
        if w(i_wlk)>0
            ...
            if w(i_wlk)>0
                ...
                if flag_mea==1
                    [e(i_wlk),proj(i_wlk)]=measure_X(H_k, Phi(:,:), Phi_T, Phi_T2, invO_matrix_up, invO_matrix_dn, invO_matrix_up2, invO_matrix_dn2, N_up, N_par, U);
                end
            end
        end
    end
