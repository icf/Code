function [E_ave,E_err,savedFileName]=CPMC_Lab_NiNi(Lx,Ly,Lz,N_up,N_dn,kx,ky,kz,U,U1,tx,ty,tz,deltau,N_wlk,N_blksteps,N_eqblk,N_blk,itv_modsvd,itv_pc,itv_Em,suffix)
...
%% Initialization
tic; % start the  timer
initialization_NiNi; % initialize internal constants, form the trial wave function and assemble the initial population of walkers
format long;
...
for i_blk=1:N_eqblk    
    for j_step=1:N_blksteps
        [Phi, w, O, E, W] = stepwlk_NiNi(Phi, N_wlk, N_sites, w, O, E, W, H_k, Proj_k_half, flag_mea, Phi_T, N_up, N_par, U, U1, fac_norm, aux_fld, aux_fld1);
...
for i_blk=1:N_blk
    for j_step=1:N_blksteps
        ...
        [Phi, w, O, E_blk(i_blk), W_blk(i_blk)] = stepwlk_NiNi(Phi, N_wlk, N_sites, w, O, E_blk(i_blk), W_blk(i_blk), H_k, Proj_k_half, flag_mea, Phi_T, N_up, N_par, U, U1, fac_norm, aux_fld, aux_fld1);
        ...
        if mod(j_step, itv_Em)==0
            ...
            % wipe out the U term, icf 2017/9/21
