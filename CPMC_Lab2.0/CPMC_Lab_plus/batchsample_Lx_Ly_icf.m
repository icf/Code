...
%% system parameters:
Lx=[2:2:8]; % The number of lattice sites in the x direction
...
for i=1:N_run
    ...
    [E_ave(i),E_err(i),savedFile]=CPMC_Lab(Lx(i),Ly,Lz,N_up,N_dn,kx,ky,kz,U,tx,ty,tz,deltau,N_wlk,N_blksteps,N_eqblk,N_blk,itv_modsvd,itv_pc,itv_Em,suffix);    
end
%% post-run:
% plot energy vs different run parameters
figure;
errorbar(Lx,E_ave,E_err);
xlabel ('L_x');
ylabel ('E');
