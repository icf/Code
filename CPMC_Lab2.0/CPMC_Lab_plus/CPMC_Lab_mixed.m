function [E_ave,E_err,C1C2_ave,C1C2_err,S1S2_ave,S1S2_err,C1C2C_ave,C1C2C_err,E_K_mixed_ave,E_K_mixed_err,E_V_mixed_ave,E_V_mixed_err,savedFileName]=CPMC_Lab_mixed(Lx,Ly,Lz,N_up,N_dn,kx,ky,kz,U,tx,ty,tz,deltau,N_wlk,N_blksteps,N_eqblk,N_blk,itv_modsvd,itv_pc,itv_Em,suffix)
...
C1C2_blk=zeros(N_blk,1);
S1S2_blk=zeros(N_blk,1);
C1C2C_blk=zeros(N_blk,1);
E_K_mixed_blk=zeros(N_blk,1);
E_V_mixed_blk=zeros(N_blk,1);
...
for i_blk=1:N_blk
    for j_step=1:N_blksteps
        ...
        [Phi, w, O, E_blk(i_blk), W_blk(i_blk), C1C2_blk(i_blk), S1S2_blk(i_blk), C1C2C_blk(i_blk), E_K_mixed_blk(i_blk), E_V_mixed_blk(i_blk)] = stepwlk_mixed(Phi, N_wlk, N_sites, w, O, E_blk(i_blk), W_blk(i_blk), C1C2_blk(i_blk), S1S2_blk(i_blk), C1C2C_blk(i_blk), E_K_mixed_blk(i_blk), E_V_mixed_blk(i_blk), H_k, Proj_k_half, flag_mea, Phi_T, N_up, N_par, U, fac_norm, aux_fld);
    ...
    C1C2_blk(i_blk)=C1C2_blk(i_blk)/W_blk(i_blk);
    S1S2_blk(i_blk)=S1S2_blk(i_blk)/W_blk(i_blk);
    C1C2C_blk(i_blk)=C1C2C_blk(i_blk)/W_blk(i_blk);
    E_K_mixed_blk(i_blk)=E_K_mixed_blk(i_blk)/W_blk(i_blk);
    E_V_mixed_blk(i_blk)=E_V_mixed_blk(i_blk)/W_blk(i_blk);
...
C1C2=real(C1C2_blk);
C1C2_ave=mean(C1C2);
C1C2_err=std(C1C2)/sqrt(N_blk);

S1S2=real(S1S2_blk);
S1S2_ave=mean(S1S2);
S1S2_err=std(S1S2)/sqrt(N_blk);

C1C2C=real(C1C2C_blk);
C1C2C_ave=mean(C1C2C);
C1C2C_err=std(C1C2C)/sqrt(N_blk);

E_K_mixed=real(E_K_mixed_blk);
E_K_mixed_ave=mean(E_K_mixed);
E_K_mixed_err=std(E_K_mixed)/sqrt(N_blk);

E_V_mixed=real(E_V_mixed_blk);
E_V_mixed_ave=mean(E_V_mixed);
E_V_mixed_err=std(E_V_mixed)/sqrt(N_blk);
