...
Ly=[2:2:20]; % The number of lattice sites in the y direction
Lz=1; % The number of lattice sites in the z direction

N_up=Lx.*Ly./2; % The number of spin-up electrons
N_dn=Lx.*Ly./2; % The number of spin-down electrons
...
for i=1:N_run
    suffix=strcat('_Nwlk',int2str(Ly(i))); % Set the suffix to distinguish between different runs in the same batch    
    % call main function AFQMC_Hub
    [E_ave(i),E_err(i),savedFile]=CPMC_Lab(Lx,Ly(i),Lz,N_up(i),N_dn(i),kx,ky,kz,U,tx,ty,tz,deltau,N_wlk,N_blksteps,N_eqblk,N_blk,itv_modsvd,itv_pc,itv_Em,suffix);    
end
%% post-run:
% plot energy vs different run parameters
figure;
errorbar(Ly,E_ave,E_err);
xlabel ('L\_y');
ylabel ('E');
