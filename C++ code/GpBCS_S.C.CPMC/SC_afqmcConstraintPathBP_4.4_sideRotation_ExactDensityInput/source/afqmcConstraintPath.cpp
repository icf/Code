...
void AfqmcConstraintPath::run()
{
    ...
    if(MPIRank() == 0){
       ...
       ostringstream oss2;
       oss2 << "GreenMatrixICFErrorBar_step_"<<0;
       greenMatrixAfterCPMCErrorBar.write(oss2.str());
    }
    if(MPIRank() == 0){
       ostringstream oss;
       oss << "DensityDensityMatrixICF_step_"<<0;
       densityDensityMatrixAfterCPMC.write(oss.str());

       ostringstream oss2;
       oss2 << "DensityDensityMatrixICFErrorBar_step_"<<0;
       densityDensityMatrixAfterCPMCErrorBar.write(oss2.str());
    }
    ...
    for(size_t i=1-1;i<=method.scSteps-1;i++){
       ...
       if(MPIRank() == 0){
          ...
          ostringstream oss2;
          oss2 << "GreenMatrixICFErrorBar_step_"<<i+1;
          greenMatrixAfterCPMCErrorBar.write(oss2.str());
       }
       if(MPIRank() == 0){
          ostringstream oss;
          oss << "DensityDensityMatrixICF_step_"<<i+1;
          densityDensityMatrixAfterCPMC.write(oss.str());

          ostringstream oss2;
          oss2 << "DensityDensityMatrixICFErrorBar_step_"<<i+1;
          densityDensityMatrixAfterCPMCErrorBar.write(oss2.str());
       }
    }
    prepareStop();
...
void AfqmcConstraintPath::runSC()
{
    //cutMinus=true;
    isBP=false;
    greenMatrixAfterCPMC.read("exactDensityMatrix.dat");
...
void AfqmcConstraintPath::initialMeasure()
{
    ...
    densityDensityMatrixAfterCPMC.resize(L2,L2); densityDensityMatrixAfterCPMC=complex <double> (0.0,0.0);
    densityDensityMatrixAfterCPMCErrorBar.resize(L2,L2); densityDensityMatrixAfterCPMCErrorBar=complex <double> (0.0,0.0);
    densityDensityMatrixAfterCPMCMeasureSave.clear();
...
{
     greenMatrixAfterCPMCMeasureSave.push_back(observeMeasure.returnGreenMatrix());  //icf: MPISum included: only at MPIrank == 0, observeMeasure.returnGreenMatrix() is valid
     densityDensityMatrixAfterCPMCMeasureSave.push_back(observeMeasure.returnDensityDensityMatrix());
...
void AfqmcConstraintPath::resetAndSaveGreenMatrix()
{
     ...
     if(densityDensityMatrixAfterCPMCMeasureSave.size() != method.writeNumber){cout<<"Error: densityDensityMatrixAfterCPMCMeasureSave.size() != method.writeNumber"<<endl;exit(1);}
     ...
     for(size_t i=1-1;i<=L2-1;i++){
         for(size_t j=1-1;j<=L2-1;j++){
            greenMatrixAfterCPMCErrorBar(i,j) = sqrt(greenMatrixAfterCPMCErrorBar(i,j)/complex<double> (greenMatrixAfterCPMCMeasureSave.size()));
         }
     } 
     greenMatrixAfterCPMCMeasureSave.clear();

     //densityDensityMatrix
     densityDensityMatrixAfterCPMC.resize(L2,L2); densityDensityMatrixAfterCPMC=complex <double> (0.0,0.0);
     densityDensityMatrixAfterCPMCErrorBar.resize(L2,L2); densityDensityMatrixAfterCPMCErrorBar=complex <double> (0.0,0.0);

     for (TensorHao< complex<double>, 2 >& n : densityDensityMatrixAfterCPMCMeasureSave){
          densityDensityMatrixAfterCPMC += n;
     }
     densityDensityMatrixAfterCPMC = densityDensityMatrixAfterCPMC/complex<double> (densityDensityMatrixAfterCPMCMeasureSave.size());

     for (TensorHao< complex<double>, 2 >& n : densityDensityMatrixAfterCPMCMeasureSave){
         for(size_t i=1-1;i<=L2-1;i++){
            for(size_t j=1-1;j<=L2-1;j++){
               densityDensityMatrixAfterCPMCErrorBar(i,j) = abs(n(i,j)-densityDensityMatrixAfterCPMC(i,j))*abs(n(i,j)-densityDensityMatrixAfterCPMC(i,j));
            }
         }   
     }
     densityDensityMatrixAfterCPMCErrorBar = densityDensityMatrixAfterCPMCErrorBar/complex<double> (densityDensityMatrixAfterCPMCMeasureSave.size());
     for(size_t i=1-1;i<=L2-1;i++){
         for(size_t j=1-1;j<=L2-1;j++){
            densityDensityMatrixAfterCPMCErrorBar(i,j) = sqrt(densityDensityMatrixAfterCPMCErrorBar(i,j)/complex<double> (densityDensityMatrixAfterCPMCMeasureSave.size()));
         }
     } 
     densityDensityMatrixAfterCPMCMeasureSave.clear();
}
