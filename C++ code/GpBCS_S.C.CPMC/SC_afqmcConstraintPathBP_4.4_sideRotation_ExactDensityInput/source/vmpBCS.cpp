...
void GpBCS_VM::setModel(Model* model_, string initialSCPhiTFlag_, TensorHao<double, 1> &tempDia)
{
    model=model_;
    ...
    if( initialSCPhiTFlag == "setFromDensity_VMGpBCS"){
       sideRotationMatrixMark=MatrixXcd::Zero(Nsite,Nsite);
       sideRotationMatrix=MatrixXcd::Zero(Nsite,Nsite);

       int tempSum=0;
       for(int i=1-1;i<=Nsite-1;i++){
       for(int j=1-1;j<=Nsite-1;j++){
          if(tempDia(2*i) == tempDia(2*j)){
          //if(abs(tempDia(2*i) - tempDia(2*j))<=0.01){
             sideRotationMatrixMark(i,j)=1.0;
             if(i >= j)tempSum++;
          }
       }
       }
...
void GpBCS_VM::updateDia(VectorXcd& v)  // a can be any unitary transformation
{
   if( initialSCPhiTFlag == "setFromDensity_VMGpBCS"){
      ...
      for(int j=1-1;j<=Nsite-1;j++){
         for(int i=1-1;i<=j-1;i++){
            if(j-1>=i && sideRotationMatrixMark(i,j) == 1.0){
               sideRotationMatrix(i,j)=v(counter)*M_PI;
               sideRotationMatrix(j,i)=-1.0*v(counter)*M_PI;
               counter++;
            }
         } 
      }
      ...
      sideRotationMatrixExp=sideRotationMatrix.exp();

      T_up=savedT_up;
      T_dn=savedT_dn*sideRotationMatrixExp;//all orbitals in rotation should be orthonormal
      for(int i=1-1;i<=Nsite-1;i++){
      for(int j=1-1;j<=Nsite-1;j++){
         T(i,2*j)=T_up(i,j);
         T(i+Nsite,2*j+1)=T_dn(i,j);
      }
      }
   }else if( initialSCPhiTFlag == "setFromGHF"){
      int counter=0;
