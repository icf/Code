...
void BCSSDOperation::reSet()
{
    ...
    densityDensityMatrixIsCalculated = false;
    ...
    densityDensityMatrixBCSBPIsCalculated = false;
...
const TensorHao<complex<double>, 2> &BCSSDOperation::returnDensityDensityMatrix()
{
    if( ~ greenMatrixIsCalculated) returnGreenMatrix();
    if( ~ cmcmMatrixIsCalculated) returnCmCmMatrix();
    if( ~ cpcpMatrixIsCalculated) returnCpCpMatrix();
    if(densityDensityMatrixIsCalculated) return densityDensityMatrix;
    
    size_t L = walkerLeft->getL();
    densityDensityMatrix.resize(L,L);densityDensityMatrix=complex <double> (0,0);  
   
    for(size_t i=1-1; i<=L-1; i++){
       for(size_t j=1-1; j<=L-1; j++){
          if(i != j)densityDensityMatrix(i,j)=greenMatrix(i,i)*greenMatrix(j,j)-greenMatrix(i,j)*greenMatrix(j,i)-cpcpMatrix(i,j)*cmcmMatrix(i,j);
          if(i == j)densityDensityMatrix(i,j)=greenMatrix(i,j);
       }
    }

    densityDensityMatrixIsCalculated = true;
...
const TensorHao<complex<double>, 2> &BCSSDOperation::returnDensityDensityMatrixBCSBP()
{
    if( ~ greenMatrixBCSBPIsCalculated) returnGreenMatrixBCSBP();
    if( ~ cmcmMatrixBCSBPIsCalculated) returnCmCmMatrixBCSBP();
    if( ~ cpcpMatrixBCSBPIsCalculated) returnCpCpMatrixBCSBP();
    if(densityDensityMatrixBCSBPIsCalculated) return densityDensityMatrix;
    ...
    densityDensityMatrixBCSBPIsCalculated = true;
