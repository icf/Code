function [E,E_ED,E_real]=Energy_X_RBM2_2(a,w,Phi_T,Proj_k,N_sites,N_y,N_up,N_dn,U,H_k)
%% Initialization
E=0;
RE=0;
...
%%
     ...
     n_phi=n_phi+1;
         ...
         ebN_up=zeros(N_sites,N_sites,N_up);
         ebN_dn=zeros(N_sites,N_sites,N_dn);
         
         temp=y2'*a;
         temp_T=temp';
         for i=1:N_sites
             for j=1:N_up
                 eN_up(i,i,j)=exp(temp_T(i,1));
             end
             for j=1:N_dn
                 eN_dn(i,i,j)=exp(-1*temp_T(i,1));
             end
         end  
         
         for i=1:N_sites
             for j=1:N_up
                 ebN_up(i,i,j)=exp(w(i,1));
             end
             for j=1:N_dn
                 ebN_dn(i,i,j)=exp(-1*w(i,1));
             end
         end    
         
         phi_1=Phi_T;
         for j=1:N_up
             phi_2(:,j,n_phi)= ebN_up(:,:,j)*eN_up(:,:,j)*phi_1(:,j);
         end
         for j=1:N_dn
             phi_2(:,N_up+j,n_phi)= ebN_up(:,:,j)*eN_dn(:,:,j)*phi_1(:,N_up+j);
         end
     end
