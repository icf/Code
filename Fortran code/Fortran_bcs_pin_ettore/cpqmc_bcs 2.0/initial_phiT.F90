...
if(PT.eq.0) then
  ...
  if(I_wavefun.eq.1)then
    if(sc_loop_flag .EQ. 0) then
       ...
       phiT_save=0
       phiT_save=phiT  !save the initial phiT in phiT_save
    endif
  endif   
...
complex(kind=8)::ph(Nsite,Ntot)
!VMpBCS initial----------------------
real(kind=8)::VMpBCS_temp
real(kind=8)::VMpBCS_phase_temp
complex(kind=8)::complex_VMpBCS_phase_temp
!var-------------
integer::var_num
real(kind=8)::var_gap
...
if (decMethod .EQ. 4)then
  !------------------------------
  !start VMpBCS_phase  !together with C++ VMpBCS_phase_main_
  !-----------------------------
  write(*,*)"start vmpbcs_phase"
  call vmpbcs_phase()
  write(*,*)"end vmpbcs_phase"


  open(unit=10,file='VMpBCS_phase_eigenvalues_up.dat',status='old')
  write(*,*)'read from VMpBCS_phase_eigenvalues_up.dat'
  do i=1,Nsite,1
     read(10,*)complex_VMpBCS_phase_temp
     ev_up(i)=abs(complex_VMpBCS_phase_temp)
  enddo
  close(10)
  open(unit=10,file='VMpBCS_phase_eigenvalues_dn.dat',status='old')
  write(*,*)'read from VMpBCS_phase_eigenvalues_dn.dat'
  do i=1,Nsite,1
     read(10,*)complex_VMpBCS_phase_temp
     ev_dn(i)=abs(complex_VMpBCS_phase_temp)
  enddo
  close(10)

  open(unit=10,file='VMpBCS_phase_eigenvectors_up.dat',status='old')
  write(*,*)'read from VMpBCS_phase_eigenvectors_up.dat'
  do i=1,Nsite,1
     do j=1,Nsite,1
        read(10,*)complex_VMpBCS_phase_temp
        cicj_sc_global_up(j,i)=complex_VMpBCS_phase_temp
     enddo
  enddo
  close(10)
  open(unit=10,file='VMpBCS_phase_eigenvectors_dn.dat',status='old')
  write(*,*)'read from VMpBCS_phase_eigenvectors_dn.dat'
  do i=1,Nsite,1
     do j=1,Nsite,1
        read(10,*)complex_VMpBCS_phase_temp
        cicj_sc_global_dn(j,i)=complex_VMpBCS_phase_temp
     enddo
  enddo
  close(10)
...
if (decMethod .EQ. 4)then
  ...
  write(*,*)'Analytic with VMpBCS_phase input'
  ev_m=0

  open(unit=10,file='VMpBCS_phase_input.inputdat',status='old')
  write(*,*)'read from input VMpBCS_phase'
  do i=1,Nsite,1
     read(10,*)VMpBCS_phase_temp
     b=(ev_up(i)+ev_dn(i))/2
     ev_m(i,i)=sqrt(b/(1-b))
     ev_m(i,i)=exp( (0,-1.0)*VMpBCS_phase_temp*acos(-1.0) )*ev_m(i,i)
     write(*,*)ev_m(i,i)
  enddo
  close(10)
...
if (decMethod .EQ. 3)then
  ...
  write(*,*)'Analytic with VMpBCS input'
  ev_m=0

  open(unit=10,file='VMpBCS_input.inputdat',status='old')
  write(*,*)'read from input VMpBCS'
  do i=1,Nsite,1
     read(10,*)VMpBCS_temp
     ev_m(i,i)=sqrt(VMpBCS_temp/(1-VMpBCS_temp))
     write(*,*)ev_m(i,i)
  enddo
  close(10)
...
!---------------------------------------------------------------------------
do i=1,Dtot,1
   if(decMethod .EQ. 999)then
      write(*,*)"phi0 = initial phiT"
      phi_sc=0
      phi_sc(1:Nsite,1:Nspin(1),i)=phiT_save(1:Nsite,1:Nspin(1),i)
      phi_sc(1:Nsite,Nspin(1)+1:Ntot,i)=phiT_save((Nsite+1):(2*Nsite),(Nspin(1)+1):Ntot,i)
   else
      write(*,*)"phi0 = decomposed CiCj"
      phi_sc=0
