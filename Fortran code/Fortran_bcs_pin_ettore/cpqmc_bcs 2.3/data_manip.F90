...
if(Neqblock .NE. 1)call openUnit(ObdmName,15,'R');
call openUnit('Obdm_error',16,'R')
call openUnit('Obdm_imag',17,'R')
call openUnit('Obdm_real',101,'R')
!call openUnit(ScorrName,20,'R')
...
!-------------------
!call openUnit(nupnuptName,62,'A')
!call openUnit(ndnnuptName,63,'A')
call openUnit(cdensityName,80,'A')
call openUnit(sdensityName,90,'A')
!-------------------
max_n=max(max_crn,0)
if(fw_bk.NE.'FW') max_n=0 !for release back propogation.
do i=0,max_n,1
...
!   call err_anal_c(var_l(1:Nsamples,i),Nsamples,var_t,var_e)
   ...
   write(*,*)"energy: e_t,e_e"
   write(*,*) e_t,e_e
   write(*,*)"energy: eE_t,eE_e"
   write(*,*) eE_t,eE_e
   ...
   do sitei=1,2*Nsite
     ...
     &                       Nsamples,zmean,err)
         if(sitei.le.Nsite.and.sitej.le.Nsite)then
           if(Neqblock .NE. 1)write(15,*)zmean
           write(101,*)dble(zmean)
           write(17,*)aimag(zmean)
           write(16,*)err
         ...
         elseif(sitei.gt.Nsite.and.sitej.gt.Nsite)then
           ...
           if(Neqblock .NE. 1)write(15,*)(0.d0,0.d0)
           write(101,*)0.d0
           write(17,*)0.d0
           write(16,*)0.d0
...
!-----------------------------------------------------------
   do beta=1,Nbands
     do alpha=1,Nbands
       write(25,*)'step: ',sc_step_counter
       do sitei=1,Nbravais
         call err_anal(ninj_l(1:Nsamples,sitei,alpha,beta,i),Nsamples,mean,err)
         write(25,*)mean
       enddo
       write(25,*)' '
     enddo
...
!-----------------------------------------------------------
   do beta=1,Nbands
     do alpha=1,Nbands
       szsz_k_pi=0
       write(26,*)'step: ',sc_step_counter
       do sitei=1,Nbravais
         call err_anal(szsz_l(1:Nsamples,sitei,alpha,beta,i),Nsamples,mean,err)
         sitex=coor(sitei,1)
         sitey=coor(sitei,2)
         szsz_k_pi=szsz_k_pi+exp((0,1)*pi*(sitex+sitey-2))*mean
         write(26,*)mean
       enddo
       szsz_k_pi=szsz_k_pi/Nbravais
       write(26,*)' '
       write(26,*)'szsz_k_pi: ',szsz_k_pi
       write(26,*)' '
     enddo
   enddo
!-----------------------------------------------------------
...
!        if(Nbeta.gt.0)write(61,*)
   ...
   write(80,*)' '
   write(80,*)'step: ',sc_step_counter
   write(80,*)' '
   write(90,*)' '
   write(90,*)'step: ',sc_step_counter
   write(90,*)' '
   do sitei=1,2*Nsite,1
      do sitej=1,2*Nsite,1
         call err_anal_c(cicj_l_global(1:Nsamples,sitei,sitej,i),Nsamples,cicj_sc_global(sitei,sitej),err_c)
   ...
   do sitei=1,Nsite,1
      write(80,*) 1-real(cicj_sc_global(sitei,sitei))-abs(cicj_sc_global(sitei+Nsite,sitei+Nsite))
      write(90,*) real( cicj_sc_global(sitei,sitei)-cicj_sc_global(sitei+Nsite,sitei+Nsite) )/2.d0
   enddo

end do
close(10)
if(Neqblock .NE. 1)then
   if(I_obdm.eq.1)close(15);
endif
if(I_obdm.eq.1)close(16)
if(I_obdm.eq.1)close(17)
if(I_obdm.eq.1)close(101)
!close(20)
!close(30)
!close(40)
!close(50)
