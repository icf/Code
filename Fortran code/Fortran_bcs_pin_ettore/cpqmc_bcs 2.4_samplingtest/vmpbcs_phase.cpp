...
namespace Hsystem_ini{
   ...
   {
   public: 
      int Dimen,Nsite,Nspin,Ntot,BC,FE_cho,Phase_lock,Dec_method,Eig_cho,ptype,Min_cho,Tot_sample_num; 
      ...
      HSystem(){
         ...
         ifstream infile1("param_vmpbcs_phase");
            ...
            infile1>>Min_cho;    //1 for mixed estimator (pBCS and N.O.); 2 for Abnormal term phase cancelling without Ci; 3 for Abnormal term phase cancelling; 4 for pure estiamtor with MC; 5 for pure estiamtor with MC concretion test.       
            infile1>>Tot_sample_num;         
         infile1.close();
...
// the hopping in the ( -1, -1 ) direction
 ...
 //get all sampled C_{kl} information
   void get_sample_C_kl(void)
   {
      int Tot_num=FNC();
      Tot_mark_value=0;
      ...
      for(int i=2-1;i<=Tot_sample_num-1;i++){
 ...
 //get all C_{kl} information
   ...
   VectorXi sample_initial_mark(void)
   {
      ...
      VectorXi mark=VectorXi::Zero(Nsite);
      for(int i=Nsite-Nspin+1-1;i<=Nsite-1;i++)mark(i)=1;  //Attention! make sure the last term is the largest
      return mark; 
   } 
   //
   ...
   //
   MatrixXcd get_mark_FE_up(VectorXi& mark)
   {
      int mark_cho=-1;     
 
      MatrixXcd FE_up_marked=MatrixXcd::Zero(Nsite,Nspin);
      for(int i=1-1;i<=Nsite-1;i++){
         if(mark(i) == 1){
            mark_cho++;
            FE_up_marked(i,mark_cho)=1.0;
         }
      }
      FE_up_marked=eigen_vectors_up*FE_up_marked;
      
      return FE_up_marked; 
   } 
   //
   MatrixXcd get_mark_FE_dn(VectorXi& mark)
   {
      int mark_cho=-1; 

      MatrixXcd FE_dn_marked=MatrixXcd::Zero(Nsite,Nspin);
      for(int i=1-1;i<=Nsite-1;i++){
         if(mark(i) == 1){
            mark_cho++;
            FE_dn_marked(i,mark_cho)=1.0;
         }
      }
      FE_dn_marked=eigen_vectors_dn*FE_dn_marked;
...
//std::cout << "rand_drop: "<<mark << std::endl;
//std::cout << "rand_drop: "<<rand_drop <<"rand_pick"<< rand_pick << std::endl;
//std::cout << "pick_mark_down: "<<pick_mark_down <<"drop_mark_down"<< drop_mark_down << std::endl;
      ...
      //srand((unsigned int)(time(NULL)));
      //std::cout << "value1/value0: " <<  value1/value0 << std::endl;
      if( double(rand())/double( RAND_MAX ) <= sqrt(value1/value0) )mark=mark_try;      
      //if( double(rand())/double( RAND_MAX ) <= 1)mark=mark_try;   
...
public:
    ...
    double get_mixed_energy_test(MatrixXcd& pBCS, MatrixXcd& FE_up, MatrixXcd& FE_dn, VectorXi& mark);

    double get_pure_energy(MatrixXcd& pBCS);

    double get_pure_sample_energy(MatrixXcd& pBCS);
//
    double operator()(const VectorXd& v) //v is the phase of pBCS eiegn_values
    {
      double energy=999.0; 
       ...
       if(Hsystem.Min_cho == 5){
          //energy=get_pure_sample_energy(pBCS);
          energy=get_pure_energy(pBCS);
       }else{
          energy=get_mixed_energy(pBCS,Hsystem.FE_up,Hsystem.FE_dn);
       }
       return energy;
    }
};
...
//V
   ...
   for(int i=1-1;i<=Hsystem.Nsite-1;i++){
    ...
    if(Hsystem.Min_cho == 4){
      ...
      //std::cout <<" Sampled total energy(abnormal): "<< energy <<" Sampled total real limit: " << Hsystem.real_limit << std::endl;  
...
// in sampling energy, we trying to test if our equation is correct
double VMpBCS_phase::get_mixed_energy_test(MatrixXcd& pBCS, MatrixXcd& FE_up, MatrixXcd& FE_dn, VectorXi& mark)
{
       MatrixXcd ovp_matrix=FE_up.transpose()*pBCS.conjugate()*FE_dn;   //<pBCS|FE>
...
//V
   ...
   for(int i=1-1;i<=Hsystem.Nsite-1;i++){
      for(int j=1-1;j<=Hsystem.Nsite-1;j++){
         for(int k=1-1;k<=Hsystem.Nsite-1;k++){
            if(mark(j) == 1 && mark(k) == 0)sum_T += Hsystem.onsiteU*conj(phase_dia(k,k)/phase_dia(j,j))*Hsystem.eigen_vectors_up(i,j)*Hsystem.eigen_vectors_dn(i,j)*conj(Hsystem.eigen_vectors_up(i,k))*conj(Hsystem.eigen_vectors_dn(i,k));
         }
      }
   } 
...
double VMpBCS_phase::get_pure_energy(MatrixXcd& pBCS)
{

      int Tot_num=Hsystem.FNC();
      double mark_sum=0;

      VectorXi mark=Hsystem.initial_mark();
      MatrixXcd FE_up=Hsystem.get_mark_FE_up(mark);
      MatrixXcd FE_dn=Hsystem.get_mark_FE_dn(mark);

      double energy=get_mixed_energy_test(pBCS,FE_up,FE_dn,mark)*Hsystem.get_mark_value(mark);
      mark_sum += Hsystem.get_mark_value(mark);
      //std::cout <<"mark:  "<<mark <<" get_mixed_energy: "<< energy <<" Hsystem.get_mark_value: "<< Hsystem.get_mark_value(mark) << std::endl;

      for(int i=2-1;i<=Tot_num-1;i++){
         Hsystem.update_mark(mark);
         FE_up=Hsystem.get_mark_FE_up(mark);
         FE_dn=Hsystem.get_mark_FE_dn(mark);
         energy +=get_mixed_energy_test(pBCS,FE_up,FE_dn,mark)*Hsystem.get_mark_value(mark);
         //std::cout <<"mark:  "<<mark <<" get_mixed_energy: "<< energy <<" Hsystem.get_mark_value: "<< Hsystem.get_mark_value(mark) << std::endl;
         mark_sum += Hsystem.get_mark_value(mark);
      }
...
double VMpBCS_phase::get_pure_sample_energy(MatrixXcd& pBCS)
{

      int Tot_num=Hsystem.Tot_sample_num;
      double mark_sum=0;

      VectorXi mark=Hsystem.sample_initial_mark();
      MatrixXcd FE_up=Hsystem.get_mark_FE_up(mark);
      MatrixXcd FE_dn=Hsystem.get_mark_FE_dn(mark);

      double energy=get_mixed_energy_test(pBCS,FE_up,FE_dn,mark)*sqrt(Hsystem.get_mark_value(mark));
      mark_sum += sqrt(Hsystem.get_mark_value(mark));
      //std::cout <<"mark:  "<<mark <<" get_mixed_energy: "<< energy <<" Hsystem.get_mark_value: "<< Hsystem.get_mark_value(mark) << std::endl;

      for(int i=2-1;i<=Tot_num-1;i++){
         Hsystem.sample_update_mark(mark);
         FE_up=Hsystem.get_mark_FE_up(mark);
         FE_dn=Hsystem.get_mark_FE_dn(mark);
         energy +=get_mixed_energy_test(pBCS,FE_up,FE_dn,mark)*sqrt(Hsystem.get_mark_value(mark));
         //std::cout <<"mark:  "<<mark <<" get_mixed_energy: "<< energy <<" Hsystem.get_mark_value: "<< Hsystem.get_mark_value(mark) << std::endl;
         mark_sum += sqrt(Hsystem.get_mark_value(mark));
      }
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    if(Hsystem.Min_cho == 4)Hsystem.get_sample_C_kl();

    if(Hsystem.Min_cho == 5){
       MatrixXcd C_kl_standard=MatrixXcd::Zero(Hsystem.Nsite,Hsystem.Nsite);
       MatrixXcd C_kl_avg=MatrixXcd::Zero(Hsystem.Nsite,Hsystem.Nsite);
       Hsystem.get_C_kl();
       C_kl_standard=Hsystem.C_kl/Hsystem.Tot_mark_value;

       //energy
       Hsystem.initial_lattice(Hsystem.BC, Hsystem.FE_up, Hsystem.FE_dn); //get the H0 and FE wave function  
       VMpBCS_phase fun_sampling_test;
       VectorXd v_test = Hsystem.ini_phase;

       VectorXd C_kl_error=VectorXd::Zero(21);
       VectorXd energy_sample=VectorXd::Zero(21);

       //for(int i=1-1;i<=20-1;i++){
       //   sleep(1);
       //   Hsystem.C_kl=MatrixXcd::Zero(Hsystem.Nsite,Hsystem.Nsite);
       //   Hsystem.Tot_mark_value=0;
       //   Hsystem.get_sample_C_kl();

       //   energy_sample(i)=fun_sampling_test(v_test);
         
       //   for(int k=1-1;k<=Hsystem.Nsite-1;k++){
       //      for(int j=1-1;j<=Hsystem.Nsite-1;j++){
       //         C_kl_error(i) += abs(Hsystem.C_kl(k,j)/Hsystem.Tot_mark_value-C_kl_standard(k,j));
       //      }
       //   }
       //   C_kl_avg += Hsystem.C_kl/Hsystem.Tot_mark_value;
       //}

       energy_sample(20)=fun_sampling_test(v_test);

       C_kl_avg=C_kl_avg/20.0;
       for(int k=1-1;k<=Hsystem.Nsite-1;k++){
          for(int j=1-1;j<=Hsystem.Nsite-1;j++){
             C_kl_error(20) += abs(C_kl_avg(k,j)-C_kl_standard(k,j));
          }
       }

       ofstream outfile11("VMpBCS_sampling_test_Ckl.dat");
       for(int i=1-1;i<=21-1;i++)outfile11<< C_kl_error(i) <<endl;
       outfile11.close();

       ofstream outfile12("VMpBCS_sampling_test_energy.dat");
       for(int i=1-1;i<=21-1;i++)outfile12<< energy_sample(i) <<endl;
       outfile12.close();
    }
    //std::cout <<" C_kl: "<< Hsystem.C_kl/Hsystem.Tot_mark_value << std::endl; 
