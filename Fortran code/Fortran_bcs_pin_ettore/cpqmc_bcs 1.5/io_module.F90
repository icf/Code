...
OPEN(UNIT=myUnit, FILE=filename, STATUS='OLD', ACTION='WRITE',POSITION='APPEND',IOSTAT=fileStatus)
IF(fileStatus>0) OPEN(UNIT=myUnit, FILE=filename, STATUS='NEW', ACTION='WRITE',IOSTAT=fileStatus)
IF(fileStatus>0) THEN
PRINT *,stopname
STOP
END IF
