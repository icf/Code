...
if(dtype .EQ. 'd')call compute_FB_d(ovlp(1:Nspin(1),1:Nspin(1),1),i,avn_up,avn_dn);
if(dtype .EQ. 'c')call compute_FB_c(i,avn_up,avn_dn)
do j=1,Nsite,1
...
subroutine compute_FB_d(Am1,i,nu,nd)
use param
use phi_param
use lattice_param
use model_param
...
complex(kind=8)::Green(2*Nsite,2*Nsite)
integer::k,p,q,j
complex(kind=8)::phu(2*Nsite,Nspin(1)),phd(2*Nsite,Nspin(2)),ph(2*Nsite,Ntot)
complex(kind=8)::Am1phut(Nspin(1),2*Nsite),Fstarphd(2*Nsite,Nspin(2))
complex(kind=8)::FDphu(2*Nsite,Nspin(1)),FDphuAm1t(2*Nsite,Nspin(1))

phu(1:Nsite,1:Nspin(1))=phi(1:Nsite,1:Nspin(1),i)
phd(Nsite+1:2*Nsite,Nspin(1)+1:Ntot)=phi(Nsite+1:2*Nsite,Nspin(1)+1:Ntot,i)
...
end subroutine compute_FB_d



subroutine compute_FB_c(i,nu,nd)
use param
use phi_param
use lattice_param
use model_param
...
complex(kind=8)::Am(Ntot,Ntot),imp

complex(kind=8)::dummy1(2*Nsite,Ntot),dummy2(Ntot,2*Nsite)


ph(1:2*Nsite,1:Ntot)=phi(1:2*Nsite,1:Ntot,i)

call bcs_over_lap_c(transpose(Fpairing),ph,Am(1:Ntot,1:Ntot))
call inverse_pf(Am(1:Ntot,1:Ntot),Ntot,imp)

!Green=matmul( matmul( Transpose(Fpairing) , ph), matmul( Am ,Transpose(ph) ) )
call zgemm('T','N',2*Nsite,Ntot,2*Nsite,one,conjg(Fpairing),2*Nsite,ph,2*Nsite,zero,dummy1,2*Nsite)
call zgemm('N','T',Ntot,2*Nsite,Ntot,one,Am,Ntot,ph,2*Nsite,zero,dummy2,Ntot)
call zgemm('N','N',2*Nsite,2*Nsite,Ntot,one,dummy1,2*Nsite,dummy2,Ntot,zero,Green,2*Nsite)

Green=transpose(Green)
do j=1,Nsite
   nu(j)=Green(j,j)
   nd(j)=Green(j+Nsite,j+Nsite)
enddo


end subroutine compute_FB_c
