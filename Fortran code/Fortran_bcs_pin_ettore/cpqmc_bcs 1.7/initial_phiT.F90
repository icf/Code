...
if(rank.eq.0) then 
  ...
  call MPI_BCAST(BCS_sc,2*Nsite*2*Nsite,MPI_DOUBLE_COMPLEX,0,MPI_COMM_WORLD,IERR)
#endif

end subroutine get_cc_phiT
    
    
subroutine input_phiT_cc()
use phiT_param
use lattice_param
use mpi_serial_param
use model_param
...
phiT(1:2*Nsite,1:Ntot,1:Dtot)=phi_sc(1:2*Nsite,1:Ntot,1:Dtot)

end subroutine input_phiT_cc
...
complex(kind=8)::cicj_sc_global_temp(2*Nsite,2*Nsite),cicj_sc_global_temp2(2*Nsite,2*Nsite)
...
real(kind=8)::ev_up(Nsite),ev_dn(Nsite),ev(2*Nsite)
complex(kind=8)::ev_m(2*Nsite,2*Nsite)
complex(kind=8)::temp_BCS_sc(2*Nsite,2*Nsite)

integer::i,j
real(kind=8):: a
complex(kind=8)::alpha,delta
complex(kind=8)::b,temp_sum
!var-------------
integer::var_num
real(kind=8)::var_gap
...
!get ev_m
   ...
   temp_BCS_sc=zero
   BCS_sc=zero
   call zgemm('N','N',Nsite,Nsite,Nsite,one,cicj_sc_global_up(1:Nsite,1:Nsite), &
&Nsite,ev_m(1:Nsite,1:Nsite),Nsite,zero,temp_BCS_sc(1:Nsite,1:Nsite),Nsite)
   call zgemm('N','T',Nsite,Nsite,Nsite,one,temp_BCS_sc(1:Nsite,1:Nsite),Nsite, &
&cicj_sc_global_dn(1:Nsite,1:Nsite),Nsite,zero,BCS_sc(1:Nsite,1:Nsite),Nsite)

!-------------------------------------------------
   phi_sc=0
   do i=1,Dtot,1
      phi_sc(1:Nsite,1:Nspin(1),i)=cicj_sc_global_up(1:Nsite,Nsite-i*Nspin(1)+1:Nsite-(i-1)*Nspin(1))
      phi_sc(Nsite+1:2*Nsite,Nspin(1)+1:Ntot,i)=cicj_sc_global_dn(1:Nsite,Nsite-i*Nspin(2)+1:Nsite-(i-1)*Nspin(2)) 
...
elseif(dtype .EQ. 'c')then
   if (a .EQ. 0) then
      cicj_sc_global(1:2*Nsite,1:2*Nsite)=(1-a)*cicj_sc_global(1:2*Nsite,1:2*Nsite)
   else
      cicj_sc_global(1:2*Nsite,1:2*Nsite)=(1-a)*cicj_sc_global(1:2*Nsite,1:2*Nsite)+a*cicj_sc_global1(1:2*Nsite,1:2*Nsite)
   endif
   cicj_sc_global=(conjg(transpose(cicj_sc_global))+cicj_sc_global)/2.0

   cicj_sc_global1(1:2*Nsite,1:2*Nsite)=cicj_sc_global(1:2*Nsite,1:2*Nsite)

   cicj_sc_global_temp=cicj_sc_global

   call eigen(cicj_sc_global_temp,2*Nsite,ev)
...
!get ev_m
   if (decMethod .EQ. 2)then
      ...
      ev_m=0
      do i=1,Nsite,1
         ev(2*i-1)=(ev(2*i-1)+ev(2*i))*0.5d0
         ev(2*i)=ev(2*i-1)
      enddo
      do i=1,2*Nsite,1
         b=ev(i)
   ...
   if (decMethod .EQ. 1)then
      write(*,*)'DET'
      ev_m=0
      do i=2*Nsite-Ntot+1,2*Nsite,1
         ev_m(i,i)=1
      enddo
   endif
   !----------------------------------------------------
   !splite the ev_m, such that the first one map the second one
   !----------------------------------------------------
   do i=1,Nsite,1
      ev_m(2*i,2*i-1)=((-1)**(i+1))*ev_m(2*i-1,2*i-1)
      ev_m(2*i-1,2*i-1)=0
      ev_m(2*i-1,2*i)=((-1)**i)*ev_m(2*i,2*i)
      ev_m(2*i,2*i)=0
   enddo
   ...
   call zgemm('N','N',2*Nsite,2*Nsite,2*Nsite,one,cicj_sc_global_temp(1:2*Nsite,1:2*Nsite), &
&2*Nsite,ev_m(1:2*Nsite,1:2*Nsite),2*Nsite,zero,temp_BCS_sc(1:2*Nsite,1:2*Nsite),2*Nsite)
   call zgemm('N','T',2*Nsite,2*Nsite,2*Nsite,one,temp_BCS_sc(1:2*Nsite,1:2*Nsite), &
&2*Nsite,cicj_sc_global_temp(1:2*Nsite,1:2*Nsite),2*Nsite,zero,BCS_sc(1:2*Nsite,1:2*Nsite),2*Nsite)
!-------------------------------------------------
   phi_sc=0
   do i=1,Dtot,1
      phi_sc(1:2*Nsite,1:Ntot,i)=cicj_sc_global_temp(1:2*Nsite,2*Nsite-i*Ntot+1:2*Nsite-(i-1)*Ntot) 
      !coe_multi(i)=sqrt(sum(ev_up(Nsite-i*Nspin(1)+1:Nsite-(i-1)*Nspin(1)))+sum(ev_dn(Nsite-i*Nspin(2)+1:Nsite-(i-1)*Nspin(2))))
   enddo
