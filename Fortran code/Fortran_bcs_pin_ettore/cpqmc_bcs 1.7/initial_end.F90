...
if(Nbands.eq.1)then
  ...
  if(abs(Hpinn).lt.0.000001)then
    ...
    if(rank.eq.0)write(*,*)'A pinning field is applied, with type:',pinningtype 
  endif
  read(10,*) pinndir
  read(10,*) onsitU
...
!sc param
!------------------
read(10,*) sc_initial_choose
read(10,*) GM_input_flag
read(10,*) sc_loop_num
read(10,*) decMethod
...
if(I_wavefun.eq.2)then
  allocate(FPairing(2*Nsite,2*Nsite))
  if(Nzeta.gt.0)then
...
if(allocated(FPairing)) deallocate(FPairing)
...
!properties to be always measured
 ...
 sig=zero;absig=zero;cicj_l_global=zero
