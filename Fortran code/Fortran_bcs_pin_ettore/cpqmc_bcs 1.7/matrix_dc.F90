...
!Get the <BCS|ph>=ovp
subroutine bcs_over_lap_d(F,ph,ovp)
use param
use lattice_param
use model_param
implicit none
complex(kind=8),intent(IN)::F(2*Nsite,2*Nsite)
complex(kind=8),intent(IN)::ph(2*Nsite,Ntot)
complex(kind=8),intent(OUT)::ovp(Nspin(1),Nspin(1))
complex(kind=8)::dummy(Nspin(1),Nsite),dummy2(Nspin(1),2*Nsite)
...
end subroutine bcs_over_lap_d


!Get the <BCS|ph>=ovp
subroutine bcs_over_lap_c(F,ph,ovp)
use param
use lattice_param
use model_param
implicit none
...
complex(kind=8)::dummy(Ntot,Nsite),dummy2(Ntot,2*Nsite)
!dummy = phi_up^ T * F^conjg
! ovp = dummy * phi_dn
dummy=0
dummy2=0
ovp=0
if(dtype .EQ. 'c')then
   call zgemm('T','N',Ntot,2*Nsite,2*Nsite,one,ph,2*Nsite,conjg(F(:,:)),2*Nsite,zero,dummy2,Ntot)
   call zgemm('N','N',Ntot,Ntot,2*Nsite,one,dummy2,Ntot,ph,2*Nsite,zero,ovp,Ntot)
endif
end subroutine bcs_over_lap_c
...
complex(kind=8),intent(IN)::ph(2*Nsite,Ntot)
complex(kind=8),intent(OUT)::imp
complex(kind=8)::ovp(Ntot,Ntot),imp_test

integer::i_deb,j_deb,i,j

if(dtype .EQ. 'd')call bcs_over_lap_d(F,ph(1,1),ovp(1:Nspin(1),1:Nspin(1)));
if(dtype .EQ. 'c')call bcs_over_lap_c(F,ph(1,1),ovp(1:Ntot,1:Ntot));

if(dtype .EQ. 'd')call inverse_d(ovp(1:Nspin(1),1:Nspin(1)),Nspin(1),imp);  
if(dtype .EQ. 'c')call inverse_pf(ovp(1:Ntot,1:Ntot),Ntot,imp); 
