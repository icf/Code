...
complex(kind=8)::cicjm_global(2*Nsite,2*Nsite)
...
integer::i_deb,j_deb
logical::i_debug
complex(kind=8)::tmp,dummy,temp_sum
...
!   enddo
   ...
   if(I_wavefun.eq.2)then
      cicj_global(1:2*Nsite,1:2*Nsite,1)=Amat_pure(1:2*Nsite,1:2*Nsite)
   elseif(I_wavefun.eq.1)then
...
if(allocated(sisj_local))deallocate(sisj_local)
if(allocated(ninj_local))deallocate(ninj_local)
if(allocated(szsz_local))deallocate(szsz_local)
if(allocated(nofr_local))deallocate(nofr_local)
if(allocated(didj_local))deallocate(didj_local)

!-----------------------------------
deallocate(cicj_global)
...
if(.not.back_pro) then
  ...
  do k=1,Dtot,1
    if(Nzeta.eq.0)then
      if(dtype .EQ. 'd')call bcs_over_lap_d(Fpairing,phR(1,1),ovp_tmp(1:Nspin(1),1:Nspin(1)));
      if(dtype .EQ. 'c')call bcs_over_lap_c(Fpairing,phR(1,1),ovp_tmp(1:Ntot,1:Ntot));
    else
      call unp_bcs_over_lap_dc(Fpairing,Dunpaired,phR(1,1),ovp_tmp(1:Nspin(1),1:Nspin(1)))
    endif

      if(dtype .EQ. 'd')call inverse_d(ovp_tmp(1:Nspin(1),1:Nspin(1)),Nspin(1),tmp);  
      if(dtype .EQ. 'c')call inverse_pf(ovp_tmp(1:Ntot,1:Ntot),Ntot,tmp);  
  ...
  do k=1,Dtot,1
     if(Nzeta.eq.0)then
       if(dtype .EQ. 'd')call bcs_over_lap_d(Fpairing,phtmp(1,1),ovp_tmp(1:Nspin(1),1:Nspin(1)));
       if(dtype .EQ. 'c')call bcs_over_lap_c(Fpairing,phtmp(1,1),ovp_tmp(1:Ntot,1:Ntot));
     else
...
!DEBUG
!  write(*,*)
!  write(*,*)sitei, Amat(sitei,sitei), Amat(sitei+Nsite,sitei+Nsite)   
!  write(*,*)sitei,Amat(sitei+Nsite,sitei),Amat(sitei,sitei+Nsite)
!  write(*,*)'Vnormal = ',Vnormal 
