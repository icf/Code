...
real(kind=8)::eps,Vext,x,y
 ...
 if(set.NE.1) then
   ...
   if(Nbands.eq.1)then
    Nhop=Nsite*2*Dimen
    !icf 5/30/2018 t' model-
    if(tprime .NE. 0 )then
       Nhop=Nsite*2*Dimen + Nsite*2*Dimen
    endif
    !-----------------------------------------------------------
...
!++|--
      ...
      !Both sides are pinned
      do mi=Nbravais-Nl(1)+1,Nbravais
        Hzero(mi,mi)=Hzero(mi,mi)+Hpinn*((-1)**(mi-(Nbravais-Nl(1))))
        Hzero(mi+Nsite,mi+Nsite)=Hzero(mi+Nsite,mi+Nsite)-Hpinn*((-1)**(mi-(Nbravais-Nl(1))))
      enddo
   elseif(pinningtype .EQ. 3)then
!+-|+-
      do mi=1,Nl(1)  !Pinning field on z axis
        Hzero(mi,mi)=Hzero(mi,mi)+Hpinn*(1)
        Hzero(mi+Nsite,mi+Nsite)=Hzero(mi+Nsite,mi+Nsite)-Hpinn*(1)
      enddo
      !Both sides are pinned
      do mi=Nbravais-Nl(1)+1,Nbravais
        Hzero(mi,mi)=Hzero(mi,mi)+Hpinn*(-1)
...
!--------------------------------------------------------------------------
!this part is added for the t-prime:the hopping in the diagonal direction, mingpu
!--------------------------------------------------------------------------
!real(kind=8)::t_prime, phase
!integer::x, y, x1, y1, l_x, l_y
if(Nhop .EQ. Nsite*2*Dimen + Nsite*2*Dimen)then
...
do i=1, Nsite, 1
   x = coor(i, 1)
   y = coor(i, 2)
   !write(*,*) "index of site", i- x - (y - 1) * l_x  
   ! write(*,*) "t1, t-prime", t1, t_prime 
!-------------------------------------------------
! the hopping in the ( 1, 1 ) direction
   ...
   sit(Nh , 1) = i
   sit(Nh , 2) = ntemp
   phase = (0.d0, 1.d0) * kbound(1) * 2.d0 * Pi / dble(l_x)
   phase = phase + (0.d0, 1.d0) * kbound(2) * 2.d0 * Pi / dble(l_y)
   hopt(Nh) = tprime * exp(phase)
   if(y1.NE.(y+1)) then
      hopt(Nh) = 0.d0
   endif
!--------------------------------------------------------------
...
! the hopping in the four directions are done
!------------------------------------------------------------------------
endif
