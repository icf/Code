...
nu=zero
call zgemm('N','T',Nspin(1),Nsite,Nspin(1),one,Am1,Nspin(1),conjg(phu),Nsite,zero,Am1phut,Nspin(1))
call zgemm('N','N',Nsite,Nspin(2),Nsite,one,Fpairing(:,:),Nsite,phd,Nsite,zero,Fstarphd,Nsite)
if(Nzeta.eq.0)then
  do j=1,Nsite
    do q=1,Nspin(2)
...
call zgemm('N','T',Nsite,Nspin(1),Nspin(1),one,FDphu,Nsite,conjg(Am1),Nspin(1),zero,FDphuAm1t,Nsite)
if(Nzeta.eq.0)then
  do j=1,Nsite
    do q=1,Nspin(2)
      nd(j)=nd(j)+FDphuAm1t(j,q)*conjg(phd(j,q))
    enddo
  enddo
else
  do j=1,Nsite
    do q=1,Nspin(2)
      nd(j)=nd(j)+FDphuAm1t(j,Nzeta+q)*conjg(phd(j,q))
    enddo
  enddo
endif
