...
if(PT.eq.0) then
  ...
  if(I_wavefun.eq.1)then
    if(sc_loop_flag .EQ. 0) then
       !if(sc_initial_choose .EQ. 0)then
          !Set from the FS wave function.
          write(*,*)'get phiT from FS'
          call set_FS_phiT()
          !Set from the HF wave function.
          !write(*,*)'get phiT from HF'
          !call set_HF_phiT()
       !elseif(sc_initial_choose .EQ. 1)then
       !   write(*,*)'get phiT from cc with input cicj_sc_global'
       !   call get_cc_phiT()
       !   sc_initial_choose=0
       !endif 
...
!------------------------------------------
!input initial wave function cicj_sc_global
!------------------------------------------
if(sc_initial_choose .EQ. 1)then
   call read_cicj_sc_global()
   !check
   write(*,*)'read_cicj_sc_global:',cicj_sc_global
   sc_initial_choose=0
endif
