...
namespace Hsystem_ini{
   ...
   {
   public: 
      int Dimen,Nsite,Nspin,Ntot,BC,FE_cho,Phase_lock,Dec_method;
      ...
      HSystem(){
         ...
         FE_cho=2; //1 for FE, 2 for cc_decomposition (Here we assuming FE_up=FE_dn which is ok for spin symmetry), 3 for 1_define matrix
      ...
      MatrixXcd read_cicj(void)
      {
         ...
         ifstream infile1("cicj_sc_global_input.inputdat");
...
{
       MatrixXcd phase_dia=MatrixXcd::Zero(Hsystem.Nsite,Hsystem.Nsite);
...
//V
   ...
   std::cout << "gggggggggggggggggg" <<energy<< "      "<< k_energy << "    "<< v_energy1 <<"     "<< v_energy2 << "  abs(v_energy2):  "<< abs(v_energy2) << std::endl;
