   ...
   public: 
      ...
      HSystem(){
         ...
         ifstream infile1("param_vmpbcs_phase");
            ...
            infile1>>Min_cho;    //1 for mixed estimator (pBCS and N.O.); 2 for Abnormal term phase cancelling without Ci; 3 for Abnormal term phase cancelling; 4 for pure estiamtor with MC       
...
// the hopping in the ( -1, -1 ) direction
 ...
 //get all sampled C_{kl} information
   void get_sample_C_kl(void)
   {
      ...
      double mark_value=1; //get_mark_value(mark);
...
//std::cout << "pick_mark_down: "<<pick_mark_down <<"drop_mark_down"<< drop_mark_down << std::endl;
      ...
      if( double(rand())/double( RAND_MAX ) <= value1/value0)mark=mark_try;      
