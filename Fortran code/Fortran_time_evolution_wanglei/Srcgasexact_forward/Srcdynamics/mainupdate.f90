...
NAMELIST /FHParams/  Thop,U,Mu,Vextin, stagger_h
...
Vext=Vextin
!----Turn off staggered magnetic field 
stagger_h=0d0
!---Copy initial state----------
do k=1,dim
phi_time(k)=gsphi(k)
...
time=0d0
write(11,'(4f15.6)') time, comup,comdo,dbocc
 ...
 DO i=1,matsize
   DO j=1,matsize
       ExpHlanc(i,j)=0.0_rKind
         DO k=1,matsize
ExpHlanc(i,j) = ExpHlanc(i,j) + Hlanc(i,k)*exp(-Xi*dt*(diff)*EigenValue(k))*Hlanc(j,k)
         END DO
   END DO
 END DO
...
!print *, 'step,diff', step, diff, comup
