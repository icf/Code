...
phiT(1:Nsite,1:Nspin(1),1)=phi_sc(1:Nsite,1:Nspin(1),1)

phiT((Nsite+1):(2*Nsite),(Nspin(1)+1):Ntot,1)=phi_sc(1:Nsite,Nspin(1)+1:Ntot,1)
...
cicj_sc_global_up=(conjg(transpose(cicj_sc_global_up))+cicj_sc_global_up)/2
cicj_sc_global_dn(1:Nsite,1:Nsite)=cicj_sc_global(Nsite+1:2*Nsite,Nsite+1:2*Nsite)
cicj_sc_global_dn=(conjg(transpose(cicj_sc_global_dn))+cicj_sc_global_dn)/2
...
enddo
do i=Nsite-Nspin(2)+1,Nsite,1
    write(*,*)'This is ev_dn from cicj_sc_global_dn, the order is',i,'the value is',ev_dn(i)
enddo

phi_sc(1:Nsite,1:Nspin(1),1)=cicj_sc_global_up(1:Nsite,Nsite-Nspin(1)+1:Nsite)
phi_sc(1:Nsite,Nspin(1)+1:Ntot,1)=cicj_sc_global_dn(1:Nsite,Nsite-Nspin(2)+1:Nsite) 
