...
do num=1,Nhilbert,1
   ...
   if(set .EQ. 3 )then 
      do i=1,Nl(1),1
         numn=num
         coe=Hcoe_up(cyci,i,i)
         coen(num)=coen(num)+coeo(numn)*dcmplx(Hpinn*((-1)**i))*dcmplx(coe)
         coe=Hcoe_dn(cycj,i,i)
         coen(num)=coen(num)+coeo(numn)*dcmplx(-1*Hpinn*((-1)**i))*dcmplx(coe)
      enddo
      do i=Nlattice-Nl(1)+1,Nlattice,1
         numn=num
         coe=Hcoe_up(cyci,i,i)
         coen(num)=coen(num)+coeo(numn)*dcmplx(Hpinn*((-1)**(1+i-(Nlattice-Nl(1)))))*dcmplx(coe)
         coe=Hcoe_dn(cycj,i,i)
         coen(num)=coen(num)+coeo(numn)*dcmplx(-1*Hpinn*((-1)**(1+i-(Nlattice-Nl(1)))))*dcmplx(coe)
      enddo
   endif
