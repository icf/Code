...
do Nei=1,Nexcit,1
   call getbasenameU()
   call openUnit(basenameU,50,'R')
   close(50)
end do
do i_U=0,12,1
   onsitU=dble(i_U)/2.d0
   !onsitU=-dble(i_U)/2.d0
   write(*,*) "onsitU=",onsitU
!For cycle U
...
!end cycle U
call openUnit(eigvname,30,'O')
do Nei=1,Nexcit,1
   read(30,*) tmpk,tmpv,tmpE
   call getbasenameU()
   call openUnit(basenameU,50,'A')
   write(50,'(4f15.8)') onsitU,tmpk,tmpv,tmpE
   close(50)
end do
close(30)
...
!--------------------------------------------------
subroutine getbasenameU()
use param
use io_module
implicit none
call createFileName(basenameU,'hubbard_lan_')
call appendBaseName(basenameU,'set_',set)
call appendBaseName(basenameU,'Nlat_',Nlattice)
call appendBaseName(basenameU,'Nhop_',Nhop)
call appendBaseName(basenameU,'D_',Dimen)
call appendBaseName(basenameU,'Nl1_',Nl(1))
call appendBaseName(basenameU,'Nl2_',Nl(2))
call appendBaseName(basenameU,'Nl3_',Nl(3))
call appendBaseName(basenameU,'k1_',3,kbound(1))
call appendBaseName(basenameU,'k2_',3,kbound(2))
call appendBaseName(basenameU,'k3_',3,kbound(3))
...
call appendBaseName(basenameU,'Nup_',Nup)
call appendBaseName(basenameU,'Ndn_',Ndn)
call appendBaseName(basenameU,'t1_',3,dble(kin))
!call appendBaseName(basename,'U_',3,onsitU)
call appendBaseName(basenameU,'Nei_',Nei)
call appendBaseName(basenameU,'.dat')
end subroutine getbasenameU
