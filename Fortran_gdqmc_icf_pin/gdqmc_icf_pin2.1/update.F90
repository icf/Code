...
integer::i,j
!--------------------
complex(kind=8)::ovlp(sc_loop_Na,sc_loop_Na,Dtot)
!--------------------
do i=1,Nwalkers,1
...
complex(kind=8),intent(INOUT)::ovlp(sc_loop_Na,sc_loop_Na,Dtot)
...
complex(kind=8),intent(INOUT)::inver(sc_loop_Na,sc_loop_Na,Dtot)
...
complex(kind=8)::det1
complex(kind=8)::phi_temp(2*Nsite,Ntot)
...
!Get the update explr_up and explr_dn with update the weight.
 if(crn.GT.0.d0) then
   ...
   if (sc_loop_flag .EQ. 1)then
       call get_BCS_green_matrix_ij(i,j,inver,nj)
   else
   !Get the um and vm
...
if (sc_loop_flag .EQ. 0)then
   ...
   explr_up=expln_up(m)
   explr_dn=expln_dn(m)
   phi_temp(1:2*Nsite,1:Ntot)=phi(1:2*Nsite,1:Ntot,i)
   do k=1,Nspin(1),1
      phi_temp(j,k)=phi(j,k,i)*(explr_up+one)
   end do
   do k=Nspin(1)+1,Ntot,1
      phi_temp(j+Nsite,k)=phi(j+Nsite,k,i)*(explr_dn+one)
   end do
   inver=0
   call over_lap_BCS_dc(phi_temp(1:2*Nsite,1:Ntot),inver(1:Nspin(1),1:Nspin(1),1)) 
   call caldet(Nspin(1),inver(1:Nspin(1),1:Nspin(1),1),det1)
   rat_h(m)=det1
endif
!------------------------------------------------------------------------------
...
!---------------------------------------
!if(real(nj(1,1,1)) .LT. 0)weight(i)=0
!if(real(nj(2,2,1)) .LT. 0)weight(i)=0

!if(weight(i) .GT. 1000)then
!write(*,*)'uuuuuuuuu',weight(i),fR
!write(*,*)'rat_h',rat_h,'rat_l',rat_l
!write(*,*)'iJ',i,j
!write(*,*)'nj',nj
!write(*,*)'inver1',inver
!       call over_lap_BCS_dc(phi(1:2*Nsite,1:Ntot,i),inver(1:Nspin(1),1:Nspin(1),1)) 
!       call caldet(Nspin(1),inver(1:Nspin(1),1:Nspin(1),1),det1)
!write(*,*)'det',det1
!if(abs(det1) .LT. 0.0001)weight(i)=0
!endif
!---------------------------------------
   ...
   if(j.LT.Nsite .AND. sc_loop_flag .NE. 1) then
...
!--------------------------------------------------
 !update inver after phi updating
 if(j.LT.Nsite .AND. sc_loop_flag .EQ. 1 ) then
    !Get inverse of the overlap
    do k=1,Dtot,1
       inver=0
       call over_lap_BCS_dc(phi(1:2*Nsite,1:Ntot,i),inver(1:Nspin(1),1:Nspin(1),k)) 
!-----------------------------
!if(i .EQ. 346 .AND. j .EQ. 2)then
!call caldet(Nspin(1),inver(1:Nspin(1),1:Nspin(1),1),det1)
!write(*,*)'uuuuuuuuu_before:',det1,weight(i),fR,x,nj
!endif
!-----------------------------
       call inverse(inver(1:Nspin(1),1:Nspin(1),k),Nspin(1))
    end do
 end if
!-------------------------------------------------
...
!-------------------------------------------------------------------
!This subroutine get the green's function matrix from nj=BCS
subroutine get_BCS_green_matrix_ij(i,j,inver,nj)
use param
use model_param
use phiT_param
...
use sc_loop_param
implicit none
complex(kind=8),intent(IN)::inver(Nspin(1),Nspin(1),Dtot)
complex(kind=8),intent(OUT)::nj(2,2,Dtot)
complex(kind=8),external::zdotu
...
if(dtype.EQ.'c') then
  ...
  do k=1,Dtot,1   
     !--------------------------------
     !GF_up=matmul(matmul(BCS_sc,phi(Nsite+1:2*Nsite,Nspin(1)+1:Ntot,i)),matmul(inver(1:Nspin(1),1:Nspin(1),k),transpose(phi(1:Nsite,1:Nspin(1),i))))
     temp_BCS_sc1=0
     temp_BCS_sc2=0 
     call zgemm('N','N',Nsite,Nspin(1),Nsite,one,BCS_sc,Nsite,&
phi(Nsite+1:2*Nsite,Nspin(1)+1:Ntot,i),Nsite,zero,temp_BCS_sc1,Nsite)
     call zgemm('N','T',Nspin(1),Nsite,Nspin(1),one,inver(1:Nspin(1),1:Nspin(1),k),Nspin(1),&
phi(1:Nsite,1:Nspin(1),i),Nsite,zero,temp_BCS_sc2,Nspin(1))
     call zgemm('N','N',Nsite,Nsite,Nspin(1),one,temp_BCS_sc1,Nsite,temp_BCS_sc2,Nspin(1),zero,GF_up,Nsite)

     !may have some problem!!!!!!!!!!
     !GF_dn=matmul(matmul(BCS_sc,phi(1:Nsite,1:Nspin(1),i)),matmul(inver(1:Nspin(1),1:Nspin(1),k),transpose(phi(Nsite+1:2*Nsite,Nspin(1)+1:Ntot,i))))
     temp_BCS_sc1=0
     temp_BCS_sc2=0 
     call zgemm('N','N',Nsite,Nspin(1),Nsite,one,transpose(BCS_sc),Nsite,&
phi(1:Nsite,1:Nspin(1),i),Nsite,zero,temp_BCS_sc1,Nsite)
     call zgemm('N','T',Nspin(1),Nsite,Nspin(1),one,transpose(inver(1:Nspin(1),1:Nspin(1),k)),Nspin(1),&
phi(Nsite+1:2*Nsite,Nspin(1)+1:Ntot,i),Nsite,zero,temp_BCS_sc2,Nspin(1))
...
end subroutine get_BCS_green_matrix_ij
!------------------------------------------------------------------
