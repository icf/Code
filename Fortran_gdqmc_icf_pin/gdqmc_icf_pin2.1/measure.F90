...
allocate(ovp_local(sc_loop_Na,sc_loop_Na,Dtot))
...
do i=1,Nwalkers,1
   ...
   if(sc_loop_flag .EQ. 1) then
       call get_BCS_meas_array(i,phL,coe,phR,ovp_local,imp_local,tot_local,w_meas,tot_meas)
   else
   ...
   do k=1,Dtot,1
      ...
      if(sc_loop_flag .EQ. 1) then
         Amat_local=0
         call cal_BCS_Amat_withovlpinv_dc(phR,ovp_local,Amat_local(1:Nsite,1:Nsite,k),Amat_local(Nsite+1:2*Nsite,Nsite+1:2*Nsite,k))
         !Amat_local(1:2*Nsite,1:2*Nsite,k)=transpose(Amat_local(1:2*Nsite,1:2*Nsite,k))
      else
...
subroutine get_BCS_meas_array(i,phL,coe,phR,ovp_local,imp_local,tot_local,w_meas,tot_meas)
use param
use lattice_param
use model_param
use project_param
...
integer,intent(IN)::i
complex(kind=8),intent(OUT)::phL(2*Nsite,Ntot,Dtot),coe(Dtot),phR(2*Nsite,Ntot)
complex(kind=8),intent(OUT)::ovp_local(sc_loop_Na,sc_loop_Na,Dtot)
complex(kind=8),intent(OUT)::imp_local(Dtot),tot_local,w_meas,tot_meas
complex(kind=8)::ovp_tmp(Ntot,Ntot),tmp,phtmp(2*Nsite,Ntot)
...
do k=1,Dtot,1
   ...
   !call inverse_d_dc(ovp_local(1:Ntot,1:Ntot,k),imp_local(k))
   !--------------------------------------------------------
   call over_lap_BCS_dc(phR(1:2*Nsite,1:Ntot),ovp_local(1:sc_loop_Na,1:sc_loop_Na,k)) 
   call caldet(sc_loop_Na,ovp_local(1:sc_loop_Na,1:sc_loop_Na,k),imp_local(k))
   call inverse(ovp_local(1:sc_loop_Na,1:sc_loop_Na,k),sc_loop_Na)
   tot_local=tot_local+conjg(coe(k))*imp_local(k)
end do
...
end subroutine get_BCS_meas_array    
