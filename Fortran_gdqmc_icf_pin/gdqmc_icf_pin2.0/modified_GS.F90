...
do m=1,Dtot,1
   if (sc_loop_flag .EQ. 1)then 
      call imp_fun_BCS_dc(phi(1,1,k),imp)
   else
...
implicit none
integer,intent(IN)::k
complex(kind=8),intent(OUT)::invop(sc_loop_Na,sc_loop_Na,Dtot)
complex(kind=8)::imp
integer::m
tot_imp(k)=zero
do m=1,Dtot,1
   !call imp_fun_dc(phiT(1,1,m),phi(1,1,k),imp)
   if (sc_loop_flag .EQ. 1)then
      call over_lap_BCS_dc(phi(1,1,k),invop(1,1,m))
      call inverse_d(invop(1:Nspin(1),1:Nspin(1),m),Nspin(1),imp)
   else
