...
complex(kind=8)::ev_m(Nsite,Nsite)
...
sum_ev=0
do i=1,Nsite,1
    ev_m(i,i)=sqrt(ev_up(i)*ev_dn(i))
    if(ev_up(i) .LT. 0 .OR. ev_dn(i) .LT. 0 )ev_m(i,i)=0
    sum_ev=sum_ev+ev_m(i,i)
    !ev_m(i,i)=sqrt(ev_m(i,i))
    !ev_m(i,i)=1
enddo
ev_m=ev_m/(sum_ev/7.d0)
