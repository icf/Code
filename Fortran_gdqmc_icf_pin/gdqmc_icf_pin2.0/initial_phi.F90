...
!Do population control, read to prob_init then control it do different walkers.
   ...
   if(PP.EQ.0) then
     !read phi_init
     !read phi_coe
     call read_phi(Nw,Nsite,Ntot,phi_coe,phi_init)
...
!-----------------------------
subroutine read_phi(Nw,Nsite,Ntot,phi_coe,phi_init)
use io_module
implicit none
...
do m=1,Dtot,1
   ...
   if (sc_loop_flag .EQ. 1)then 
      call imp_fun_BCS_dc(phi_init(1,1),imp)
   else
