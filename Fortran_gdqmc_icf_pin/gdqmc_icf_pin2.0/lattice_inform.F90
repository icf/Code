...
integer::mi,mj
 ...
 do mi=1,Nhop,1
    ...
    !---------------------------------
    ! add pinning field, icf, 1/2/2018
    !---------------------------------
 do mi=1,Nsite,1
    if(coor(mi,1) .EQ. 1 )then
       Hzero(mi,mi)=Hzero(mi,mi)+0.25*(-1)**coor(mi,2)
       Hzero(mi+Nsite,mi+Nsite)=Hzero(mi+Nsite,mi+Nsite)+(-1*0.25)*(-1)**coor(mi,2)
    endif
    if(coor(mi,1) .EQ. Nl(1))then
       Hzero(mi,mi)=Hzero(mi,mi)-0.25*(-1)**coor(mi,2)
       Hzero(mi+Nsite,mi+Nsite)=Hzero(mi+Nsite,mi+Nsite)-(-1*0.25)*(-1)**coor(mi,2)
    endif
 end do
...
do i=1,Nsite,1
   do j=1,Dimen,1
      ...
      if(coor(i,j).EQ.1) then
          ...
          !---------------------------
          !cut off at the small Y edge, icf, 1/2/2018
          !---------------------------
          if(j .NE. 1) then
             ntemp=(Nl(j)-1)*den+i
             Nh=Nh+1
...
!---------------------------
!cut off at the small Y edge, so Nh=Nhop-2*N1(2), icf, 1/2/2018
!---------------------------
Nhop=Nhop-2*Nl(2)
if(Nh.ne.Nhop) then
  write(*,*) "Something is wrong with Nhop Pinning"
  call mystop
end if
end subroutine sethopPinning
